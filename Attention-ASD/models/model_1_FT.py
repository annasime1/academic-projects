...
class FocalLoss(nn.Module): 
    def __init__(self, alpha=torch.tensor([0.2046286 , 0.11357086, 0.07076175, 0.21085573, 0.16390283,
       0.18359415, 0.05268607]), gamma=2.0, reduction='mean'):
...
class MLP(nn.Module):
    ...
    def __init__(self, mean, std):
        ...
        self.augment = DataAugmentationLayer(mean, std, noise_std_factor=0.1)
        self.criterion_gaze = FocalLoss()
    ...
    def predict(self, x):
        ...
        if self.eval:
          ...
          if(max_class < 0.8):
            ...
            # print(output)
            new_output = output.clone().detach()  
            new_output[0][2] = 1000
            # print('\nNuovo output:\n')
            # print(torch.argmax(new_output, dim=1))
            return torch.argmax(new_output, dim=1)
          else:
