...
%% Get files
lista = dir(strcat(direct,'\',patient)); %patient in generale sono le cartelle, dentro sono numeri da 1 al numero di cartelle
...
ficheiroang = strcat(direct,'\',patient,'\',anglesfiles{1}) %ficheiro vuol dire file. file angoli
...
for im=1:length(tempos_labels)
    m(im) = find(temposf==tempos_labels(im)); %salvo gli indici di temposf che hanno match con tempos_label
end
